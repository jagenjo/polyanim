    ...
    onMouse(e,editor) {
        ...
        if(e.type === "pointerdown")
        {
            ...
            if(!this.drawingShape)
            {
                ...
                {
                    frame.add( shape )
                    state.shape = frame.shapes.indexOf(shape)
                }
                else
                {
        ...
        else if(e.type === "pointermove" && this.lastPos && e.buttons )
        {
            ...
            if(shape)
            {
                const dist = distance(localpos,this.lastPos)
                if(dist > this.minDist)
                {
                    shape.points.push([localpos[0],localpos[1]])
                    this.lastPos[0] = localpos[0]
                    this.lastPos[1] = localpos[1]
                }
                state.point = -1
                return true
            }
...
POLYANIM.Editor.tools.push( DRAW_TOOL )